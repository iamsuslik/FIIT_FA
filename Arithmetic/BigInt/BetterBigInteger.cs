...
public sealed class BetterBigInteger : IBigInteger
{
    ...
    {
        // Предполагаем, что |a| >= |b|, результат всегда будет иметь размер a.Length
        ...
        for (int i = 0; i < a.Length; i++)
        {
            ...
            // Вычитаем сначала заем, потом второе число
            // Сначала обработаем заем
            uint sub1 = valA - borrow;
            uint b1 = (valA < borrow) ? 1u : 0u; // Если valA < borrow, значит был заем

            // Теперь вычитаем b[i]
            uint finalSub = sub1 - valB;
            uint b2 = (sub1 < valB) ? 1u : 0u; // Если sub1 < valB, значит был заем

            result[i] = finalSub;
            borrow = b1 + b2; // Новый заем — это сумма обоих заемов
        }
    ...
    private uint[] ToTwoComplement(int targetLength)
    {
        ...
        // Остальные заполняем нулями (для отрицательных чисел при инверсии они станут 1)
        ...
        if (IsNegative)
        {
            ...
            // 2. Прибавляем 1 (реализация через uint с проверкой переноса)
            uint carry = 1u;
            for (int i = 0; i < targetLength && carry > 0; i++)
            {
                uint old = res[i];       
                res[i] = old + carry;
                // Если новое значение меньше старого, значит был перенос
    ...
    private static BetterBigInteger FromTwoComplement(uint[] data)
    {
        ...
        else
        {
            ...
            // 1. Вычитаем 1 (реализация через uint с проверкой заема)
            uint borrow = 1u;
            for (int i = 0; i < res.Length && borrow > 0; i++)
            {
                uint old = res[i];
                res[i] = old - borrow;
                // Если результат стал больше, значит был заем
    ...
    public static BetterBigInteger operator <<(BetterBigInteger a, int shift)
    {
        ...
        // Проверка на случай, если число 0
        ...
        for (int i = 0; i < aDigits.Length; i++)
        {
            ...
            // Текущий элемент = сдвинутое значение | перенос из предыдущего
            result[i + wordShift] = (current << bitShift) | carry;
        
            // Вычисляем перенос для следующей итерации
            // Если bitShift == 0, переноса нет, иначе сдвигаем вправо на (32 - bitShift)
            carry = (bitShift == 0) ? 0u : (current >> (32 - bitShift));
        }

        // Если после цикла остался хвост переноса, записываем его
